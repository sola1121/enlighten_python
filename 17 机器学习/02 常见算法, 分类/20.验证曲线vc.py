...
data = []
with open(os.path.dirname(__file__) + '/car.txt', 'r') as f:
...
train_scores1, test_scores1 = ms.validation_curve(model, x, y, 'n_estimators', n_estimators, cv=5)
...
train_scores2, test_scores2 = ms.validation_curve(model, x, y, 'max_depth', max_depth, cv=5)
...
mp.fill_between(n_estimators, train_means1 - train_stds1,train_means1 + train_stds1, color='dodgerblue',alpha=0.25)
mp.fill_between(n_estimators, test_means1 - test_stds1,test_means1 + test_stds1, color='orangered',alpha=0.25)

mp.plot(n_estimators, train_means1, 'o-', c='dodgerblue', label='Training')
mp.plot(n_estimators, test_means1, 'o-', c='orangered', label='Testing')
mp.legend()
...
mp.fill_between(max_depth, train_means2 - train_stds2,train_means2 + train_stds2, color='dodgerblue',alpha=0.25)
mp.fill_between(max_depth, test_means2 - test_stds2,test_means2 + test_stds2, color='orangered',alpha=0.25)
