...
dates, closing_prices = np.loadtxt(os.path.dirname(__file__) + "/aapl.csv", delimiter=',',
...
ma52 = np.convolve(closing_prices,np.ones(5) / 5, 'valid')

weights = np.exp(np.linspace(-1, 0, 5))
weights /= weights.sum()

ma53 = np.convolve(closing_prices, weights[::-1], 'valid')
...
mp.plot(dates[4:], ma51, c='orangered',linewidth=1, label='MA-51')
mp.plot(dates[4:], ma52, c='orangered',alpha=0.25, linewidth=5, label='MA-52')
